...
def func_premier_parent_dispo(par_matrice_parent, par_nb_parent, par_plage, par_tab_pris): # renvoie le premier parent dispo pour la plage donnée (par_plage) 
    for num_parent in range(par_nb_parent):
        if par_tab_pris[num_parent] == 1 and par_matrice_parent[num_parent][par_plage] == 1:
            return num_parent

    return (-1)

def func_main(par_matrice_prof, par_matrice_parent, par_nb_plage, par_nb_parent, par_tab_pris):
    for num_plage in range(par_nb_plage):
        #Plage_dispo_prof = func_premiere_dispo_prof(Matrice_prof, nb_plage)
        if par_matrice_prof[0][num_plage] == -1:
            Parent_dispo_meme_plage = func_premier_parent_dispo(par_matrice_parent, par_nb_parent, num_plage, par_tab_pris)

            if Parent_dispo_meme_plage != -1 :
                Matrice_prof[0][num_plage] = Parent_dispo_meme_plage
                par_tab_pris[Parent_dispo_meme_plage] = 0

def func_affichage_rdv(par_nb_plage, par_tab_corres_plage_heure, par_matrice_prof):
    for num_plage in range(par_nb_plage):
        #afficher l'horaire du créneau
        creneau = par_tab_corres_plage_heure[num_plage]
        parent_pour_creneau = str(par_matrice_prof[0][num_plage])
...
Tab_pris = [1]*nb_parent    # 0 -> le parent n'est pas dispo / 1 -> le parent est dispo 
...
func_main(Matrice_prof, Matrice_parent, nb_plage, nb_parent, Tab_pris)

print("")

func_affichage_rdv(nb_plage, Tab_corres_plage_heure, Matrice_prof)
